...
class PCSFInput(object):
    def __init__(self,prizeFile,edgeFile,b,mu,dummyMode,knockout,garnet,gb,shuffle,musquared):
        """
        ...
        #    threads = 1
        try:
            print ('Continuing with parameters w = %f, b = %f, D = %i, mu = %f, g = %f.')%(float(w), float(b), int(D), mu, g)
        except:
...
class PCSFOutput(object):
    ...
    def writeCytoFiles(self, outputpath, outputlabel, cyto30, i,j):
        """
        ...
        if cyto30:
            ...
            #Cytoscape 3.0
            optSif= open('%s/%s_optimalForest_%d_%d.sif'%(outputpath,outputlabel,i,j), 'wb')
            augSif = open('%s/%s_augmentedForest_%d_%d.sif'%(outputpath,outputlabel,i,j), 'wb')
            dumSif = open('%s/%s_dummyForest_%d_%d.sif' %(outputpath, outputlabel,i,j), 'wb')
        
            #Write attribute files in a format supported by Cytoscape
            #The first lines of these files contains the variable names
            noa = open('%s/%s_nodeattributes_%d_%d.tsv'%(outputpath, outputlabel,i,j), 'wb')
            noa.write('Protein\tPrize\tBetweennessCentrality\t'\
                      'FractionOfOptimalForestsContaining\n')
            eda = open('%s/%s_edgeattributes_%d_%d.tsv'%(outputpath, outputlabel,i,j), 'wb')
...
def seq(start, stop, step=1):
    ...
    if n > 1:
        ...
        return([])
def main():
    prizeFile = 'terminal_set_1_2.txt'
    edgeFile = 'edgeFile.txt'
    confFile = 'confFile.txt'
    dummyMode = './sig_terminal_set_1_2.txt'
    garnet = None
    garnetBeta = '0.01'
    muSquare = False
    msgPath = './msgsteiner/msgsteiner.exe'
    knockout = '[]'
    k_cv = 2 # k-fold cross validation
    cv_reps = None # how many times you want to run cross validation
    outputPath = '.'
    outLabel = 'result'
    cyto30 = True
    shuffleNum = 0
    seed = 0
    noiseNum = 1 #how many times we want to add noise to edges
    termNum = 0 # how many times we want to apply given values to random nodes
    #pdb.set_trace()
    beta = seq(1,20,1)
    ...
    for i in range(0,len(beta)):
        for j in range(0,len(mu)):
            inputObj = PCSFInput(prizeFile,edgeFile, beta[i], mu[j], dummyMode,
                         knockout, garnet, garnetBeta, shuffleNum,
                         muSquare)
    
            (edgeList, info) = inputObj.runPCSF(msgPath, None)
            outputObj = PCSFOutput(inputObj,edgeList,info,outputPath,outLabel,1)
            outputObj.writeCytoFiles(outputPath, outLabel, cyto30,i,j)
        #Get merged results of adding noise to edge values
            if noiseNum > 0:
                merged = changeValuesAndMergeResults(noiseEdges, seed, inputObj, 
                                             noiseNum, msgPath, 
                                             outputPath, outLabel)
                merged.writeCytoFiles(outputPath, outLabel+'_noisy', cyto30,i,j)
    
    #Get merged results of shuffling prizes
            if shuffleNum > 0:
               merged = changeValuesAndMergeResults(shufflePrizes, seed, inputObj, 
                                             shuffleNum, msgPath, 
                                             outputPath, outLabel)
               merged.writeCytoFiles(outputPath, outLabel+'_shuffled', cyto30)

    #Get merged results of randomizing terminals
            if termNum > 0:
               merged = changeValuesAndMergeResults(randomTerminals,seed, inputObj,
                                             termNum, msgPath,
                                             outputPath, outLabel)
               merged.writeCytoFiles(outputPath, outLabel+'_randomTerminals', 
                              cyto30)
    
    #If k is supplied, run k-fold cross validation
            if k_cv != None:
                if cv_reps == None:	
                    crossValidation(k_cv, 1, inputObj, seed, msgPath, \
                            outputPath, outLabel)
                else:
                    for i in range(0,cv_reps):
                        crossValidation(cv, i+1, inputObj, seed, msgPath, \
	                            outputPath, outLabel)
